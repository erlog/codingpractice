...
require_relative 'c_optimization'
...
def random_tri(size)
    a = PointObject.new(rand(size), rand(size), rand(size)).to_f!
    b = PointObject.new(rand(size), rand(size), rand(size)).to_f!
    c = PointObject.new(rand(size), rand(size), rand(size)).to_f!
    return [a,b,c].sort
end
