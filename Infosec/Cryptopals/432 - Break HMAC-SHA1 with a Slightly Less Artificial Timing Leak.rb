...
while true 
	...
	break if (response == 200) or (signature.length > 39)

	responsetimes = Hash.new{[]}
	numberofrepetitions.times do	
		signaturechars.each do |char|
			...
			responsetimes[char] = responsetimes[char] << responsetime
		end
	end

	responsetimes.keys.each do |key|
		responsetimes[key] = responsetimes[key].inject(:+)/numberofrepetitions
	end

	signature += responsetimes.key(responsetimes.values.max) 
	print signature[-1]
end
puts
