...
void setup() {
    ...
    image_elements = parse_element_file("TextElements.xml");
    text_elements = parse_element_file("ImageElements.xml");
}

void draw() {
    //render
    render_layer(background_elements, background_layer);
    render_layer(image_elements, text_layer);
    render_layer(text_elements, image_layer);

    //composite
    image(background_layer, 0, 0);
    //image(image_layer, 0, 0);
    image(text_layer, 0, 0, width, height, 0, 0, width, height, SOFT_LIGHT);
}
